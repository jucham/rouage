...
///
/// \class SparseGraph SparseGraphs.h
/// \brief This class implements sparse graph with generic node and edge
///
///
...
const edge_type* SparseGraph<node_type, edge_type>::GetEdge(int from, int to)const
{
       ...
       ++curEdge)
  {
   ...
   oss << "CONST GET EDGE : From =  " << from << ", To = " << to << std::endl;
...
edge_type* SparseGraph<node_type, edge_type>::GetEdge(int from, int to)
{
       ...
       ++curEdge)
  {
   ...
   std::ostringstream oss;
   oss << "GET EDGE : From =  " << from << ", To = " << to << std::endl;
//   Logger::write("logAI.txt", oss.str());
...
//  Use this to add an edge to the graph. The method will ensure that the
//  edge passed as a parameter is valid before adding it to the graph. If the
//  graph is NOT a digraph then a similar edge connecting the nodes in the opposite
//  direction will be automatically added.
//-----------------------------------------------------------------------------
template <class node_type, class edge_type>
...
void SparseGraph<node_type, edge_type>::SetEdgeCost(int from, int to, float NewCost)
{
  ...
  assert ( (from < m_Nodes.size()) && (to < m_Nodes.size()) && "<SparseGraph::SetEdgeCost>: invalid index");
...
template <class node_type, class edge_type>
void SparseGraph<node_type, edge_type>::render()
{
   ...
   for (unsigned int nodeIdx = 0; nodeIdx < m_Nodes.size(); ++nodeIdx)
   {
         ...
         curEdge!=m_Edges[nodeIdx].end(); ++curEdge)
      {
         ...
         //Vector2D fromNodePos = m_Nodes[nodeIdx]->LinkedTile()->Position();
         //Vector2D toNodePos = m_Nodes[curEdge->To()]->LinkedTile()->Position();

         // renders more large line if edge is bidirectional
         curEdge->render( *this );
