...
public:
	...
	void PacketSent(int size)
	{
		if (sentQueue.exists(local_sequence))
		{
			...
			for (PacketQueue::iterator itor = sentQueue.begin();
	...
	{
		return generate_ack_bits(GetRemoteSequence(), receivedQueue,
				max_sequence);
	}

	void ProcessAck(unsigned int ack, unsigned int ack_bits)
	{
		process_ack(ack, ack_bits, pendingAckQueue, ackedQueue, acks,
				acked_packets, rtt, max_sequence);
	}
	...
	static unsigned int generate_ack_bits(unsigned int ack,
			const PacketQueue & received_queue, unsigned int max_sequence)
	{
		unsigned int ack_bits = 0;
		for (PacketQueue::const_iterator itor = received_queue.begin();
	...
	// TODO creer un variable pour contenir la liste des sequences acked
	const std::list<unsigned int> & GetAckedSequences() const {
		static std::list<unsigned int> ackedSequences;
		ackedSequences.clear();
		for (PacketQueue::const_iterator itor = ackedQueue.begin(); itor != ackedQueue.end(); itor++) {
			ackedSequences.push_back(itor->sequence);
		}
		return ackedSequences;
	}
...
protected:
	...
	void AdvanceQueueTime(float deltaTime)
	{
		...
		for (PacketQueue::iterator itor = receivedQueue.begin();
				itor != receivedQueue.end(); itor++)
			itor->time += deltaTime;

		for (PacketQueue::iterator itor = pendingAckQueue.begin();
				itor != pendingAckQueue.end(); itor++)
			itor->time += deltaTime;

		for (PacketQueue::iterator itor = ackedQueue.begin();
	...
	void UpdateQueues()
	{
		...
		while (sentQueue.size()
				&& sentQueue.front().time > rtt_maximum + epsilon)
			sentQueue.pop_front();
